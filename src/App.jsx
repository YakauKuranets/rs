...
export default function App() {
  ...
  const fetchFtpRoot = async (serverAlias, path = "/") => {
    // 1. СРАЗУ ОТКРЫВАЕМ ОКНО И СТАВИМ АКТИВНЫЙ СЕРВЕР
    ...
    try {
        ...
        // 3. Если ошибка - показываем ее, но окно остается открытым
        alert(`Сбой подключения к ${serverAlias.toUpperCase()}:\n${err}`);
        setFtpItems([]); // Очищаем список файлов
    } finally {
        setLoading(false);
    }
  };
